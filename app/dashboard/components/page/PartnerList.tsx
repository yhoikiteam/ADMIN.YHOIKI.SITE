...
import {
  ...
  FaBoxOpen, // Total Items Added
  FaEnvelopeOpenText, // Icon for Official Letter
  FaUsers // For Total Partners overview
} from 'react-icons/fa';
...
interface Partner {
  ...
  email: string;
  phone: string;
  type: 'Reseller' | 'Seller' | 'Mentor'; // Tipe Partnership
  status: 'Active' | 'Inactive' | 'Pending';
  joinDate: string; // Tanggal bergabung partnership
  address?: string;
  notes?: string;
  totalSalesValue?: number; // Contoh data ringkasan
  totalItemsAdded?: number; // Contoh data ringkasan
  officialLetterStatus: 'Belum Dikirim' | 'Dikirim' | 'Ditandatangani' | 'Ditolak'; // Status surat resmi partnership
}
...
function PartnerForm({ onSubmit, initialData, onCancel }: { onSubmit: (data: Partner | Omit<Partner, 'id'>) => void; initialData?: Partner | null; onCancel: () => void }) {
    const [formData, setFormData] = useState<Omit<Partner, 'id'> | Partner>(
      initialData || {
        ...
        totalSalesValue: 0,
        totalItemsAdded: 0,
        officialLetterStatus: 'Belum Dikirim', // Default status for new partners
      }
    );
    ...
    useEffect(() => {
      if (initialData) {
        ...
        setFormData({
          ...
          totalSalesValue: 0,
          totalItemsAdded: 0,
          officialLetterStatus: 'Belum Dikirim',
        });
      }
    }, [initialData]);
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="name" className="block text-sm font-medium text-gray-700">Nama Partnership</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label htmlFor="status" className="block text-sm font-medium text-gray-700">Status Partnership</label>
        ...
        {/* New Field for Official Letter Status */}
        <div>
            <label htmlFor="officialLetterStatus" className="block text-sm font-medium text-gray-700">Status Surat Resmi Partnership</label>
            <select id="officialLetterStatus" name="officialLetterStatus" value={formData.officialLetterStatus} onChange={handleChange} required className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">
...
// --- Main PartnerListPage Component ---

export default function PartnerListPage() { // Renamed from MitraPage to PartnerListPage
  const [partners, setPartners] = useState<Partner[]>([
    { id: 'partner1', name: 'Dimas F. Utama', contactPerson: 'Dimas F. Utama', email: 'dimas@example.com', phone: '081211112222', type: 'Reseller', status: 'Active', joinDate: '2023-01-10', address: 'Jakarta', notes: 'Reseller aktif produk digital', totalSalesValue: 15000000, totalItemsAdded: 50, officialLetterStatus: 'Ditandatangani' },
    { id: 'partner2', name: 'PT Solusi Abadi', contactPerson: 'Umar A.', email: 'umar.a@solusiabadi.com', phone: '081333334444', type: 'Seller', status: 'Active', joinDate: '2023-03-20', address: 'Bandung', notes: 'Menjual layanan IT', totalSalesValue: 75000000, totalItemsAdded: 10, officialLetterStatus: 'Ditandatangani' },
    { id: 'partner3', name: 'Anggun Aini', contactPerson: 'Anggun Aini', email: 'anggun@mentor.com', phone: '085755556666', type: 'Mentor', status: 'Active', joinDate: '2023-05-15', address: 'Surabaya', notes: 'Mentor coding bootcamp', totalSalesValue: 0, totalItemsAdded: 0, officialLetterStatus: 'Dikirim' },
    { id: 'partner4', name: 'Siti K. Jaya', contactPerson: 'Siti K. Jaya', email: 'siti@gamejaya.com', phone: '087877778888', type: 'Seller', status: 'Pending', joinDate: '2024-01-01', address: 'Semarang', notes: 'Menunggu verifikasi produk game', totalSalesValue: 0, totalItemsAdded: 0, officialLetterStatus: 'Belum Dikirim' },
    { id: 'partner5', name: 'Budi Santoso', contactPerson: 'Budi Santoso', email: 'budi.s@reseller.net', phone: '082199990000', type: 'Reseller', status: 'Inactive', joinDate: '2023-10-01', address: 'Yogyakarta', notes: 'Reseller non-aktif, perlu follow-up', totalSalesValue: 5000000, totalItemsAdded: 20, officialLetterStatus: 'Ditolak' },
  ]);
  ...
  const _addPartner = (newPartner: Omit<Partner, 'id'>) => {
    const id = `partner${partners.length + 1}-${Date.now()}`;
    const partnerToAdd = { ...newPartner, id };
    setPartners((prev) => [...prev, partnerToAdd]);
    addNotification('success', `Partnership "${newPartner.name}" berhasil didaftarkan.`);
    if (newPartner.status === 'Pending') {
      addNotification('info', `Partnership baru "${newPartner.name}" sedang menunggu verifikasi.`);
    }
  };
  ...
  const _updatePartner = (updatedPartner: Partner) => {
    ...
    addNotification('success', `Data partnership "${updatedPartner.name}" berhasil diperbarui.`);
  };

  const deletePartner = (id: string, name: string) => {
    if (confirm(`Apakah Anda yakin ingin menghapus partnership "${name}" ini?`)) {
      setPartners((prev) => prev.filter((p) => p.id !== id));
      addNotification('warning', `Partnership "${name}" berhasil dihapus.`);
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <h1 className="text-4xl font-bold text-gray-900 mb-8 text-center">Manajemen Partnership Yhoiki</h1>

      {/* Overview Statistics (Similar to Dashboard/Pedanaan) */}
      <div className="bg-white rounded-2xl shadow-xl p-6 mb-8 max-w-7xl mx-auto">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">Ringkasan Partnership</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="bg-blue-50 p-6 rounded-lg shadow-sm flex items-center justify-between">
            <div>
              <p className="text-sm text-blue-700">Total Partnership Aktif</p>
          ...
          <div className="bg-purple-50 p-6 rounded-lg shadow-sm flex items-center justify-between">
            <div>
              <p className="text-sm text-purple-700">Partnership Menunggu Verifikasi</p>
          ...
          <div className="bg-yellow-50 p-6 rounded-lg shadow-sm flex items-center justify-between">
            <div>
              <p className="text-sm text-yellow-700">Total Partnership Terdaftar</p>
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          >
            <FaPlusCircle className="mr-2" /> Tambah Partnership Baru
          </button>
        </div>
        ...
        <div className="overflow-x-auto bg-white rounded-xl shadow p-4">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Partnership</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kontak</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipe Partnership</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status Partnership</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status Surat Resmi</th> {/* New Column for Official Letter */}
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ringkasan Aktivitas</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredPartners.map((partner) => (
                <tr key={partner.id} className="hover:bg-gray-50">
                  ...
                  {/* New Cell for Official Letter Status */}
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          partner.officialLetterStatus === 'Ditandatangani' ? 'bg-green-100 text-green-800' :
                          partner.officialLetterStatus === 'Dikirim' ? 'bg-blue-100 text-blue-800' :
                          partner.officialLetterStatus === 'Ditolak' ? 'bg-red-100 text-red-800' :
                          'bg-gray-100 text-gray-800'
                      }`}>
                          <div className="flex items-center">
                              <FaEnvelopeOpenText className="mr-1" /> {partner.officialLetterStatus}
                          </div>
                      </span>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-700">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-8 shadow-2xl max-h-[90vh] overflow-y-auto w-full max-w-2xl transform transition-all scale-100 opacity-100">
            <h3 className="text-2xl font-bold text-gray-800 mb-6 text-center">
              {editingId ? 'Edit Data Partnership' : 'Daftarkan Partnership Baru'}
            </h3>
            <PartnerForm
